...
#books = gr.getreadinglist(user=goodreads_userid, shelf="fiction")
...
sellers = filter( lambda s: len(set([L.edition.book for L in s.listings])) > 2, sellers )
sellers = sorted( sellers, key=lambda s: len(set([L.edition.book for L in s.listings])), reverse=True )
